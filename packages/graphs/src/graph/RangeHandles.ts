...
export default class RangeHandles {
  ...
  activate = (dygraph: Dygraph): { didDrawChart: () => void } => {
    this.g = dygraph;
  ...
  private placeHandle = (): void => {
    ...
    if (this.g?.getOption('showRangeSelector')) {
      ...
      if (this.g?.getOptionForAxis('drawAxis', 'x')) {
        xAxisLabelHeight =
          this.g?.getOption('xAxisHeight') ||
          this.g?.getOption('axisLabelFontSize') + 2 * this.g?.getOption('axisTickSize');
      }
      if (plotArea) {
      ...
      if (handles && handles.length > 0 && this.rangebarCanvasRect && this.singleZoomHandle) {
        ...
        if (leftHandle.style.left !== '' && rightHandle.style.left !== '') {
          ...
          if (rightHandlePos - leftHandlePos < 2) {
            const halfHandleWidth = this.singleZoomHandle.width / 2;
            this.rangeSize =
              parseFloat(rightHandle.style.left) +
              halfHandleWidth -
              (parseFloat(leftHandle.style.left) + halfHandleWidth);
            ...
            const handleTop = Math.max(
              this.rangebarCanvasRect.y,
              this.rangebarCanvasRect.y + (this.rangebarCanvasRect.h - this.singleZoomHandle.height) / 2,
            );
